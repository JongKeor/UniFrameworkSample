...
namespace UniFramework.Editor
{
	...
	public class AssetBundleBuildInfo
	{
		...
		public string AssetBundleName {
			get {
				return   UniEditorUtil.MakeRelative (fileInfo.FullName, AssetBundleBuilder.GetAssetBundlePath(buildTarget, outputPath)).Replace(".manifest","")  ;
			}
		}
		...
		public bool IsMenifest {
			get {
				return AssetBundleUtil.GetPlatformForAssetBundles(buildTarget).Equals(AssetBundleName);
			}
		}
...
//					EditorGUILayout.EndVertical ();
		...
		private void OnGUIFileInfo (AssetBundleBuildInfo info)
		{
			...
			string relativePath = UniEditorUtil.MakeRelative (info.FullPath, AssetBundleBuildSetting.AbsoluteOutputPath);
		...
		public void OnGUIMenu (int window)
		{
			...
			EditorGUILayout.LabelField ("Build Target", UniEditorUtil.CreateGUIStyle (11, new Color (0.8f, 0.8f, 0.8f, 1.0f), FontStyle.Bold));
			EditorGUILayout.LabelField (EditorUserBuildSettings.activeBuildTarget.ToString ());
			EditorGUILayout.LabelField ("Output Path", UniEditorUtil.CreateGUIStyle (11, new Color (0.8f, 0.8f, 0.8f, 1.0f), FontStyle.Bold));
			...
			if (GUILayout.Button ("Find")) {
				...
				if (!string.IsNullOrEmpty (result)) {
					string relativePath = UniEditorUtil.MakeRelative (result, System.Environment.CurrentDirectory);
			...
			if (GUILayout.Button ("Build")) {
				List<string> changedList =  AssetBundleBuilder.BuildAssetBundle (EditorUserBuildSettings.activeBuildTarget, AssetBundleBuildSetting.RelativeOutputPath);
				foreach(var c in changedList)
				{
					var ret =  this.assetBundleList.Find(o=> o.Equals(c));
					if(ret != null)	{
						ret.IsChanged = true;
					}
					else {
						var path = AssetBundleBuilder.GetAssetBundlePath(EditorUserBuildSettings.activeBuildTarget ,AssetBundleBuildSetting.RelativeOutputPath );
						var fileInfo = new System.IO.FileInfo(path);
						this.assetBundleList.Add(new AssetBundleBuildInfo(fileInfo,EditorUserBuildSettings.activeBuildTarget,AssetBundleBuildSetting.RelativeOutputPath ));
					}
				}
			}
			GUI.color = Color.white;
			EditorGUILayout.LabelField ("PublishSetting", UniEditorUtil.CreateGUIStyle (11, new Color (0.8f, 0.8f, 0.8f, 1.0f), FontStyle.Bold));
